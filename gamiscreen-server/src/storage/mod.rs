...
pub enum StorageError {
    /// Connection pool error (transient; suitable for retry).
    #[error("connection pool: {0}")]
    Pool(String),

    /// Database query or schema error.
    #[error("database: {0}")]
...
impl Store {
    ...
    pub async fn pending_submissions_count(&self) -> Result<i64, StorageError> {
        ...
        tokio::task::spawn_blocking(move || -> Result<i64, StorageError> {
            let mut conn = pool.get().map_err(|e| StorageError::Pool(e.to_string()))?;
            configure_sqlite_conn(&mut conn)?;
            let c: i64 = ts::task_submissions.count().get_result(&mut conn)?;
            Ok(c)
        })
        .await?
    }
    ...
    pub async fn process_usage_minutes(
        ...
        if minutes.is_empty() {
            ...
            return Err(StorageError::InvalidInput("no minutes provided".into()));
        }
        let pool = self.pool.clone();
