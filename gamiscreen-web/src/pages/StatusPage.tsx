...
export function StatusPage() {
  const [rows, setRows] = useState<Array<{ child: ChildDto, remaining: number, balance: number, blocked: boolean }>>([])
  ...
  async function load() {
    ...
    try {
      ...
      if (claims?.role === 'child') {
        ...
        setRows([{ child: { id: claims.child_id, display_name: claims.child_id }, remaining: rem.remaining_minutes, balance: rem.balance, blocked: rem.blocked_by_tasks }])
      } else {
        const cs = await listChildren()
        const rems = await Promise.all(cs.map(c => getRemaining(c.id)))
        setRows(cs.map((c, i) => ({ child: c, remaining: rems[i].remaining_minutes, balance: rems[i].balance, blocked: rems[i].blocked_by_tasks })))
      }
    } catch (e: any) {
              ...
              <div className="col" style={{ gap: 4 }}>
                ...
                <div
                  style={{
                    fontSize: '1.25rem',
                    fontWeight: 700,
                    color: balance < 0 ? '#b91c1c' : undefined,
                  }}
                  aria-label={balance < 0 ? `Balance: negative ${Math.abs(balance)} minutes` : undefined}
                >
                  {balance > 0 ? '+' : ''}{balance} min
                </div>
              </div>
            </div>
            {blocked && (
              ...
              >
                [!] Blocked: required tasks incomplete
              </div>
            )}
          </div>
        ))}
